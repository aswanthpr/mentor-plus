...
export class MenteeController implements IMenteeController {
  ...
  async getRefreshToken(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await this._menteeService.BLRefreshToken(req.cookies?.refreshToken);
      console.log('Result from BLRefreshToken:', result);
  ...
  async getMeneeProfile(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({ success: false, message: "Internal server error" });
      throw new Error(
        `Error while get mentee  profile data ${error instanceof Error ? error.message : String(error)
        }`
      );
    }
  }
  async getMenteeProfileEdit(req: Request, res: Response): Promise<void> {
    try {
      ...
      res.status(500).json({ success: false, message: "Internal server error" });
      throw new Error(
        `Error while get mentee  profile editing ${error instanceof Error ? error.message : String(error)
        }`
      );
    }
  }
  ...
  async getPasswordChange(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await this._menteeService.blPasswordChange(currentPassword, newPassword, _id);
      res.status(result?.status).json(result);
    } catch (error: unknown) {
      res.status(500).json({ success: false, message: "Internal server error" });
      throw new Error(
        `Error while get mentee  profile password changing ${error instanceof Error ? error.message : String(error)
        }`
      );
    }
  }
  async getProfileChange(req: Request, res: Response): Promise<void> {
    try {
      ...
      const result = await this._menteeService.blProfileChange(profileImage, _id);
      res.status(result.status).json(result)
    } catch (error: unknown) {
      res.status(500).json({ success: false, message: "Internal server error" });
      throw new Error(
        `Error while get mentee  profile changing ${error instanceof Error ? error.message : String(error)
        }`
      );
    }
  }
  ...
  async getExploreData(req: Request, res: Response): Promise<void> {
    try {
      const result = await this._menteeService.blExploreData()
    } catch (error:unknown) {
      res.status(500).json({ success: false, message: "Internal server error" });
      throw new Error(
        `Error getting mentor data in explore ${error instanceof Error ? error.message : String(error)
        }`
      );
    }
  }
